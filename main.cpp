...
private:
  ...
  void createDescriptorSet() {
    ...
    if (vkAllocateDescriptorSets(device, &allocInfo, descriptorSets.data()) !=
      ...
      throw std::runtime_error(
          "echec de l'allocation d'un set de descripteurs!");
    }
  ...
  void recreateSwapChain() {
    ...
    createFramebuffers();
    createUniformBuffers();
    createDescriptorPool(); // Ajouter cette ligne
    createDescriptorSet();  // Ajouter cette ligne
    createCommandBuffers();
  }
  ...
  void createCommandBuffers() {
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
      ...
      if (vkBeginCommandBuffer(commandBuffers[i], &beginInfo) != VK_SUCCESS) {
        throw std::runtime_error(
            "erreur au début de l'enregistrement d'un command buffer!");
      }
      ...
      if (vkEndCommandBuffer(commandBuffers[i]) != VK_SUCCESS) {
        throw std::runtime_error(
            "échec de l'enregistrement d'un command buffer!");
      }
    }
  }
  ...
  void createGraphicsPipeline() {
    ...
    colorBlendAttachment.dstColorBlendFactor =
        VK_BLEND_FACTOR_ZERO;                                       // Optionnel
    ...
    */
    ...
    pipelineInfo.renderPass =
        renderPass; // Render-pass auquel se rattache le pipeline

    pipelineInfo.subpass = 0; // index du sous-pass (le 1ᵉʳ)

    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo,
                                  nullptr, &graphicsPipeline) != VK_SUCCESS) {
      throw std::runtime_error(
          "échec de la création de la pipeline graphique!");
    }
  ...
  void createLogicalDevice() {
    ...
    if (vkCreateDevice(physicalDevice, &createInfo, nullptr, &device) !=
        VK_SUCCESS) {
      throw std::runtime_error(
          "Échec lors de la création d'un logical device!");
    }
  ...
  QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
    ...
    for (const auto &queueFamily : queueFamilies) {
      ...
      // 🔧 ici on utilise bien le paramètre `device`
  ...
  */
  ...
  void createInstance() {
    if (enableValidationLayers && !checkValidationLayerSupport()) {
      throw std::runtime_error(
          "Validation layers are enabled but aren't available!");
    }
    ...
    // 🔧 Ajout correct des extensions AVANT l'appel à vkCreateInstance
    auto extensions = getRequiredExtensions();
    createInfo.enabledExtensionCount = static_cast<uint32_t>(extensions.size());
    createInfo.ppEnabledExtensionNames = extensions.data();
    // Ajout optionnel des layers de validation
    ...
    // Juste informatif (peut être supprimé)
